...
- **Bench deadlock fixed** (`782e7f2e18`): `DebugBenchmarkManager` is `@MainActor` and blocked main inside the synchronous host-queue hop of `loadStateSlot` while the emulation loop dispatch_synced to main → watchdog kill after every rapid stop→boot. Now async + 30 s timeout; 8/8 reboot cycles survived on Chibi-Robo. Any other `DOLHostQueueRunSync` caller on the main actor has the same hazard (`DOLDebugBridge.pause/resume/frameAdvance` are called from the server queue, so they are fine).
