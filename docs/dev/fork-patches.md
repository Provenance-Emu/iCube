...
| `CachedInterpreter.{h,cpp}` InterpretBcx (2026-09-17) | +55/-2 | Conditional-branch terminal: `bcx` (opcode 16) block terminals are emitted as `InterpretBcx` (same `InterpretOperands` payload as `Interpret<true>`) and inlined in the executor's callback chain; same math as `Interpreter::bcx` minus branch watch, so only emitted when debugging is off. Chibi-Robo core 5: `Interpreter::bcx` 8.2 % → 0, executor absorbed ~5 points, net ≈ 3 points of the CPU thread. | Take upstream then re-apply; if upstream changes bcx semantics (BO decode, LK), mirror them. |
| `CachedInterpreter.{h,cpp}` InterpretBx / InterpretBclr (2026-09-17) | +70/-0 | Same treatment as InterpretBcx for the other two block terminals: `bx` (opcode 18) and `bclr` (19/16). Wind Waker core 5: `bclrx`+`bx` generic handlers (3.1 %) gone. | Take upstream then re-apply. |
| `DOLDebugBridge` + `DebugAPIRoutes` hot-blocks (2026-09-17) | +20/-0 | `GET /api/debug/hot-blocks?top=N` returns `CIRProfiler::BuildHotBlocksReport` (needs `Main.Core.CIRProfile=true` at boot). WARNING: the profiler's Record() costs ~85 % of the CPU thread while on — capture, then turn it off. | App-side; carry as-is. |
