...
void Metal::StateTracker::PrepareRender()
{
  ...
  if (is_gx)
  {
    ...
    if (!m_flags.has_gx_ps_uniform)
    {
      ...
      // Cannot batch indices 0 and 2 using a single contiguous range without clobbering
      // the VS uniform that may be bound at fragment index 1.  Use individual calls instead.
