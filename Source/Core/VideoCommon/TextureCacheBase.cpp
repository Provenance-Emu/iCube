                                           ...
                                           const std::array<u32, 3>& filter_coefficients)
{
      ...
      src_texture->GetFormat() == AbstractTextureFormat::RGBA8)
  {
    MathUtil::Rectangle<int> dst_rect(0, 0, static_cast<int>(entry->texture->GetWidth()), static_cast<int>(entry->texture->GetHeight()));
    MathUtil::Rectangle<int> src_rect_fb = framebuffer_rect;
    if (g_gfx->TryComputeGammaRGBA8(entry->texture.get(), dst_rect, src_texture, src_rect_fb, 1.0f / gamma))
    {
      entry->texture->FinishedRendering();
      g_gfx->GenerateMipmaps(entry->texture.get());
      return;
      ...
      src_texture->GetFormat() == AbstractTextureFormat::RGBA8)
  {
    ...
    const bool need_filter = AllCopyFilterCoefsNeeded(filter_coefficients) || CopyFilterCanOverflow(filter_coefficients);
    if (need_filter || apply_gamma || clamp_top || clamp_bottom)
    {
      ...
      const u32 relative_top = (top_coord_local > static_cast<u32>(framebuffer_rect.top)) ? (top_coord_local - framebuffer_rect.top) : 0u;
      const u32 relative_bottom = (bottom_coord_local > static_cast<u32>(framebuffer_rect.top)) ? (bottom_coord_local - framebuffer_rect.top) : 0u;
