...
private:
  ...
  public:
    MixerFifo(Mixer* mixer, u32 sample_rate_divisor,
              u32 sample_rate_dividend = FIXED_SAMPLE_RATE_DIVIDEND, bool is_primary = false)
        : m_mixer(mixer), m_input_sample_rate_dividend(sample_rate_dividend),
          m_input_sample_rate_divisor(sample_rate_divisor), m_is_primary(is_primary)
    {
    }
    void DoState(PointerWrap& p);
  ...
  MixerFifo m_dma_mixer{this, FIXED_SAMPLE_RATE_DIVIDEND / 32000, FIXED_SAMPLE_RATE_DIVIDEND,
                        /*is_primary=*/true};
