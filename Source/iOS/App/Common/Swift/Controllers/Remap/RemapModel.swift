...
/// iOS-only controller navigation for the remap screen: d-pad / left stick
/// move the highlighted row, A activates, B goes back. A private copy of the
/// D18 (`MenuScreen`) move/activate primitives, kept pure so it is testable and
/// so the swap to `MenuScreen` later is a rename.
///
...
/// - A and B are latch-and-rearm: one event per physical press, re-armed only
///   once the button reports released — the GameController framework can call
///   `valueChangedHandler` more than once per press, and polling sees a held
///   button on every tick.
struct RemapControllerNav: Equatable {
  struct Config: Equatable {
  ...
  /// Adopt the current physical state without emitting anything. Called when
  /// a capture session ends: the button or stick that was just captured is
  /// still held, and must not be read again as "activate" or "move".
