  ...
  var body: some View {
    List {
      if isGC {
        ...
        SelectRow(label: L("GameCube Controller"), checked: selected == 1) { selected = 1; DOLConfigBridge.setGCPortDeviceForPort(portOneBased, device: 1) }
      } else {
        // 0: None, 1: Emulated
        SelectRow(label: L("<Nothing>"), checked: selected == 0) { selected = 0; DOLConfigBridge.setWiimoteSourceFor(portOneBased, source: 0) }
        SelectRow(label: L("Emulated Wii Remote"), checked: selected == 1) { selected = 1; DOLConfigBridge.setWiimoteSourceFor(portOneBased, source: 1) }
      }
    }
