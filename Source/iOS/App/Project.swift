...
let schemes: [Scheme] = [
    ...
    .scheme(
        ...
        // LLDB init file generated by SetUpPython.sh (absolute path): installs the TXM JIT
        // bless hook (dolphin_jit_lldb.py) so Xcode's LLDB authorizes the JIT region on iOS 26.
        runAction: .runAction(configuration: .configuration("Release (Non-Jailbroken)"), customLLDBInitFile: "Derived/lldbinit"),
    ...
    .scheme(
        ...
        // LLDB init file generated by SetUpPython.sh (absolute path): installs the TXM JIT
        // bless hook (dolphin_jit_lldb.py) so Xcode's LLDB authorizes the JIT region on iOS 26.
        runAction: .runAction(configuration: .configuration("Release (Jailbroken)"), customLLDBInitFile: "Derived/lldbinit"),
