...
- (NSString*) PVAppGroupId {
    return [[NSBundle mainBundle] infoDictionary][@"APP_GROUP_IDENTIFIER"] ?: @"group.org.dolphin.DolphiniOS";
}
...
- (NSUserActivity *)handoffActivity {
    if (!_handoffActivity) {
//        _handoffActivity = [[NSUserActivity alloc] initWithActivityType:@"org.dolphin.webserver"];
        _handoffActivity = [[NSUserActivity alloc] initWithActivityType:NSUserActivityTypeBrowsingWeb];
        _handoffActivity.title = @"DolphiniOS file manager";
...
- (GCDWebUploader *)webServer
{
    if (!_webServer)
    {
        ...
        _webServer.title = @"DolphiniOS";
        _webServer.prologue = @"<p>Upload ROM files for your games here.</p><p>Supported systems: GameCube, Wii.</p>";
        _webServer.epilogue = @"<p>Check out <a href=\"https://github.com/OatmealDome/dolphin-ios\">DolphiniOS on GitHub</a></p>";
        _webServer.footer = @"<p>DolphiniOS WebUploader</p>";

        // Add custom handlers to track upload progress
        [self setupUploadProgressTracking];

        // Create user activity for handoff
        self.handoffActivity = [[NSUserActivity alloc] initWithActivityType:@"org.dolphin.webserver"];
        self.handoffActivity.title = @"DolphiniOS Web Server";
...
- (BOOL)startServers {
    ...
    if (!success) {
        NSLog(@"Error: Failed to start WWW server on %@", self.IPAddress);
        return NO;
    }

    success = [self startWebDavServer];
    if (!success) {
        NSLog(@"Error: Failed to start webdav server on %@", self.IPAddress);
    ...
    NSLog(@"Started servers on %@", self.IPAddress);
...
-(BOOL)startWWWUploadServer {
...
#endif
                                      GCDWebServerOption_ServerName : @"DolphiniOS",
                                      GCDWebServerOption_BonjourName : @"DolphiniOSWWW",
    ...
    BOOL success = [self.webServer startWithOptions:webSeverOptions
                                              error:&error];
    if (!success) {
      NSLog(@"Error: Failed to start Web Server on %@, with error: %@", self.IPAddress, error.localizedDescription);
    }
  ...
  NSLog(@"Started web server at %@", self.URL);
    return success;
}
...
-(BOOL)startWebDavServer {
...
#endif
                                         GCDWebServerOption_ServerName : @"DolphiniOS",
                                         GCDWebServerOption_BonjourName : @"DolphiniOS",
    ...
    BOOL success = [self.webDavServer startWithOptions:webDavSeverOptions
                                            error:&error];
    if (!success) {
        NSLog(@"Error: Failed to start WebDAV Server with error: %@", error.localizedDescription);
    }
...
- (void)webServerDidCompleteBonjourRegistration:(GCDWebServer*)server {
  NSLog(@"Bonjour registration completed for URL: %@", server.bonjourServerURL.absoluteString);
