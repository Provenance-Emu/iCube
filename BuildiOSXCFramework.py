...
def _safe_job_count():
    ...
    The -Ofast build has a handful of very heavy TUs (implot_items, glslang, shader gen)
    that each peak at multiple GB; running -j<cores> OOM-kills them intermittently and
    fails the build nondeterministically. Capping by memory makes builds reliable.
    """
    cores = multiprocessing.cpu_count()
    try:
        ram_bytes = os.sysconf("SC_PHYS_PAGES") * os.sysconf("SC_PAGE_SIZE")
        ram_gb = ram_bytes / (1024 ** 3)
        mem_cap = max(2, int(ram_gb // 4))
        return max(2, min(cores, mem_cap))
...
class DolphinBuilder:
    ...
    def __init__(self, verbose: bool = False, clean: bool = False, static: bool = True):
        ...
        self.repo_root_dir = self.script_dir
    ...
    def build_platform(self, platform: str) -> bool:
        ...
        if platform in ["SIMULATORARM64", "SIMULATOR_TVOS"]:
            ...
            if platform == "OS64":
                ...
                # Let ios-cmake handle Catalyst deployment target via -DPLATFORM=MAC_CATALYST and -DDEPLOYMENT_TARGET
                pass
        ...
        if platform in ["SIMULATORARM64", "SIMULATOR_TVOS"]:
            ...
            cmake_cmd.append(f"-DCMAKE_OSX_SYSROOT={macos_sdk}")
        ...
        if platform in ["SIMULATORARM64", "SIMULATOR_TVOS"]:
            ...
            if platform == "OS64":
                ...
                # Do not set CMAKE_OSX_DEPLOYMENT_TARGET for Catalyst; ios-cmake derives proper -target ...-macabi
                pass
        ...
        try:
            ...
            # Cap parallelism by available RAM, not just core count: the aggressive -Ofast flags
            # (+ -funroll-loops/-fsplit-lto-unit) make a few heavy TUs (implot_items, glslang,
            # the VideoCommon shader gen) consume multiple GB each. At -j<cores> they OOM-kill
            # intermittently and fail the build at random spots. ~4 GB/job keeps it deterministic.
            jobs = _safe_job_count()
            if self.has_ninja:
                self._log(f"Building {platform} with Ninja (-j {jobs})...", "build")
                build_cmd = [self.ninja_path, "-j", str(jobs)]
            else:
                self._log(f"Building {platform} with CMake build (-j {jobs})...", "build")
                build_cmd = [self.cmake_path, "--build", ".", "-j", str(jobs)]
